...
type Logger interface {
	// Log format and prints in console received arguments
	Log(...any)
}

// ErrorConvertor defines a interface to convert errors in a http response
type ErrorConvertor interface {
	// ConvertError converts an error in a status code and also a object to response
	ConvertError(error) (int, *response.Common)
}

// ErrorHandler defines a error handler to manage the internal errors
type ErrorHandler interface {
	// HandleError handle err and parse it in a code.Code to then translate it in an integer status
...
//
// NOTE: the error handler also implements the ErrorConverter interface
